...
class PDF(FPDF):
    ...
    def add_image(self, image_file, width=170):
        ...
        if os.path.exists(caminho_completo):
            # Adiciona margem para centralizar se a imagem for menor que a largura da página
            page_width = self.w - self.l_margin - self.r_margin
            x_pos = (page_width - width) / 2
            self.image(caminho_completo, x=self.l_margin + x_pos, w=width)
...
def gerar_pdf():
    ...
    # --- Exercício 1 ---
    pdf.chapter_title("Exercício 1: Classificação de Rolamentos")
    pdf.section_title("a) Árvore de Decisão Gerada:")
    pdf.add_image("ex1_arvore.png", width=180)
    ...
    # --- Exercício 2 ---
    pdf.add_page()
    pdf.chapter_title("Exercício 2: Classificação de Flores Íris")
    pdf.section_title("a) Árvore de Decisão Gerada:")
    pdf.add_image("ex2_arvore.png", width=180)
    pdf.section_title("b) Regras SE-ENTÃO Extraídas:")
    ...
    # --- Salvar o PDF ---
    nome_arquivo_pdf = "Relatorio_Segundo_Trabalho_Arvore_Decisao.pdf"
...
if __name__ == "__main__":
    if NOME_ALUNO == "Nome do Aluno Não Encontrado":
        print(
